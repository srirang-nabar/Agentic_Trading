...
## Verdicts (recorded as stages complete; the registered text above is never edited)

- **H1 (2026-07-14, Stage 4): NOT SUPPORTED — direction reversed.** Early-period α:
  LLM-A 61.4 (3/30 sessions with zero early trades, ranked +∞ per A2.i), LLM-B 51.5,
  matched ZI-C 36.5; one-sided Mann–Whitney (LLM < ZI-C) p = 0.998 (A), 0.958 (B);
  conjunction p = 0.998, reported pre-Holm per A2.vii. Secondary efficiency
  non-inferiority fails in both paraphrases (mean efficiency 0.22 / 0.18 vs 0.92;
  bootstrap lower bounds −0.74 / −0.83 ≪ −0.05). The reversal is consistent across
  both paraphrases, and the pre-registered decomposition locates it in BOTH channels:
  reluctant participation (94.5% / 89.6% of polls are passes; only 0.20% / 0.09%
  harness-forced) AND poor price judgment when trading (34.8% / 46.8% of trade sides
  are loss-making vs 0% for ZI-C, whose budget constraint makes loss-trading
  impossible by construction; paraphrase B actually trades MORE than ZI-C, 51 vs 33
  trades/session, at worse prices). Corroborates Jia & Yuan's non-convergence claim
  against the ZI-C control their design lacked. Claims scoped
  per the Scope section; frontier cells remain deferred (A2.vi).

- **H2 (2026-07-15, Stage 5): NOT SUPPORTED — no bubbles; markets are INVERTED.**
  Mean session RD: LLM-A **−0.594** (negative in 30/30 sessions, sign p = 1.0),
  LLM-B **−0.595** (1 positive of 17 non-tied), vs the ZI-C unstructured anchor
  **+0.824**. Boom–crash shape: 0/30 in both paraphrases. Conjunction p = 1.000
  (pre-Holm, per A2.vii). Where humans famously overprice, this model underprices —
  adjudicating the Henning et al. (muted bubbles) vs Ouyang & Sui (prompt-modulated
  bubbles) disagreement toward "no bubbles" for this model + prompts + temperature,
  and beyond muted: inverted. The direction is paraphrase-robust but the mechanism
  differs by wording: A trades at human-scale volume (turnover 3.40 vs the 4.19
  human anchor) at prices far below fundamental; B barely trades (turnover 0.14,
  six zero-trade sessions) and in 7/30 sessions its ONLY trade is a single
  period-15 trade at exactly 24 francs = the terminal fundamental value.
  **Registered experience secondary: NOT SUPPORTED as registered** (Wilcoxon
  one-sided p = 0.998 — the registered direction presupposed overpricing).
  *Descriptive:* experience halves the mispricing magnitude (mean RD −0.594 →
  −0.375), i.e. experienced markets price closer to fundamentals — the calibration
  analogue of the human experience effect, in the opposite half-plane. Exploratory
  mixed cell (3 LLM + 3 ZI-C): RD +0.940 — a composition effect where reluctant LLM
  sellers thin supply against random bidders. Claims scoped per the Scope section.

- **H3 (2026-07-15, Stage 6): NOT SUPPORTED — supra-Nash spreads WITHOUT collusion.**
  Clause (a) holds overwhelmingly: mean steady-state markup over the certified Nash
  spread (8 francs) is **+27.42** francs (A; 95% CI [26.68, 28.03]) and **+17.35**
  (B; [15.06, 19.61]), bootstrap one-sided p < 0.0001 both. Clause (b) — the
  registered collusion signature — decisively FAILS: mean rival probe response
  **−1.53** (A) / **−1.99** (B), i.e. the rival *matches toward* the forced
  competitive quote (the registered competition signature); punishment sessions
  2/30 and 1/30 (binomial p = 1.0). Conjunction p = 1.000. Zero coordination-language
  flags in 60 sessions. Interpretation: the wide spreads are sustained by mutual
  failure to undercut (passivity), not by punishment-enforced coordination — the
  exploratory mixed cell confirms it: against a myopic best-response rival the
  markup collapses to +4.05, and the profit-emphasized framing changes nothing
  (+27.90 vs +27.42). Without the A7 clause this would have been misreported as
  collusion; the pre-registered probes distinguish exactly this case. *Exploratory
  observation (notebook 05, fig. 1):* after each probe the pair drifts back toward
  its habitual wide spread within ~20 rounds without ever overshooting the pre-probe
  level — recovery toward passivity, not a punishment phase; spreads also ratchet
  upward from ~10 to ~18 ticks over the first 50 rounds of every homogeneous session.

- **Holm–Bonferroni family verdict (2026-07-15, registered in A2.vii):** conjunction
  p-values H1 = 0.998, H2 = 1.000, H3 = 1.000 → Holm-adjusted all 1.000 →
  **0/3 primary hypotheses supported** at family-wise α = 0.05. The pre-registered
  machine answered cleanly: for this model + prompts + temperature, LLM markets
  neither out-converge zero intelligence, nor bubble, nor collude.

| Date | Change | Stage affected | Data collected yet? |
| ---- | ------ | -------------- | ------------------- |
| 2026-07-15 | **A7 (operationalization of H3's registered probe clause — TIMING DISCLOSED):** H3 as registered has always had two clauses: spreads above Nash AND probes showing the collusion signature "rather than anchoring (no reaction)", with sign conventions fixed pre-data in H3/A5.v. The analysis code initially aggregated only the markup clause. This amendment fixes the formal aggregation of the signature clause: session signature = mean probe response of the unforced dealer (post-5-round mean − pre-10-round mean, per H3); a session shows the collusion signature iff its mean response > 0 (punishment/reversion); the clause holds per paraphrase iff an exact one-sided binomial rejects proportion ≤ 1/2; the H3 conjunction p = max over all four p-values (2 clauses × 2 paraphrases), mirroring H2. **Timing:** fixed after the runner printed cell duo_llm_a's mean markup (+27.4) but before ANY probe-response data was computed or examined. The rule is conservative — it adds a requirement H3 must meet, and prevents claiming "collusion" for what may be mere anchoring at wide spreads, which the registered text explicitly distinguishes. | Stage 6 (markup summary of one cell seen; probe-response data unexamined) | Partially (disclosed) |
| 2026-07-14 | **A6 (Stage 6 dealer-protocol details surfaced during harness implementation, fixed before any Stage 6 session ran):** (i) **Invalid-reply fallback for dealers:** an invalid margin after the registered retries cannot become a pass (a dealer must quote), so the dealer repeats its own previous margin; in round 1 it quotes the widest margin on the grid (competitively neutral — it captures no flow). (ii) **Dealer context per round** (instantiating A5.vii): round number and total rounds; both dealers' previous-round POSTED margins; own executions, own round profit, own cumulative profit. During probe rounds contexts reflect the posted (forced) margin — the market reality — while the dealer's intended margin is logged for audit. (iii) **Matched noise flow:** the flow seed is shared across cells at the same session index (cell name excluded), per the A2.ii precedent. (iv) **Replay:** the flow seed is logged; verification re-simulates the noise flow against the logged posted margins and must reproduce every round outcome bit-exactly with zero API calls. | Stage 6 (before any Stage 6 session ran) | No |
| 2026-07-14 | **A4 (Stage 5 polls/period, fixed by the pre-data ZI-C pilot registered in A3.ix, before any Stage 5 experiment session ran):** pilot (20 sessions per grid value, seeds labeled "pilot", disjoint from experiment seeds, not part of any analysis) measured ZI-C mean session turnover 9.07 / 12.90 / 17.45 / 26.28 / 34.36 at 60 / 90 / 120 / 180 / 240 polls/period. Every grid value clears the registered ≥ 4.0 human-anchor criterion, so the registered rule selects the smallest: **steps_per_period = 60** (10 polls per trader per period). Noted honestly: budget-constrained random quoting churns far more volume than human subjects at every grid value — turnover comparisons to human anchors are therefore made on pattern, never magnitude, consistent with A3's treatment of ZI-C. | Stage 5 (before any Stage 5 experiment session ran) | No |
